...
class MutualTrap extends PairwiseForce {
  ...
  toString(idMap?: Map<BasicElement, number>): string {
    ...
    ref_particle = ${
      idMap ? idMap.get(this.ref_particle) : this.ref_particle.id
    }
    stiff = ${this.stiff}
    r0 = ${this.r0}
    PBC = ${this.PBC}
      ...
      notify(
        `${this.description()} includes a particle that no longer exists`,
        "alert",
        true
      );
      return "";
  ...
  update() {
    ...
    this.eqDists = [p1, p1.clone().add(dir.clone().multiplyScalar(this.r0))];
    ...
    let force_v = dir
      .clone()
      .normalize()
      .multiplyScalar((dir.length() - this.r0) * this.stiff);
    dir.normalize();
    this.force = [p1, p1.clone().add(dir.multiplyScalar(force_v.length()))];
  }
}
...
class SkewTrap extends PairwiseForce {
  ...
  update() {
    ...
    let force_v = dir
      .clone()
      .normalize()
      .multiplyScalar((dir.length() - this.r0) * this.stdev);
    dir.normalize();
    this.force = [p1, p1.clone().add(dir.multiplyScalar(force_v.length()))];
  }
}
...
class ForceHandler {
  ...
  forceColors: THREE.Color[] = [
    //add more if you implement more forces
  ...
  set(forces: Force[]) {
    ...
    this.traps = <PairwiseForce[]>(
      forces.filter((f) => this.knownForces.includes(f.type))
    );
    ...
    forceGeoms.forEach((g, i) =>
      g.addAttribute("position", new THREE.Float32BufferAttribute(v1[i], 3))
    );
    let materials = this.types.map(
      (t, i) => new THREE.LineBasicMaterial({ color: nucleosideColors[i] })
    );
    this.forceLines = <THREE.LineSegments[]>(
      forceGeoms.map((g, i) => new THREE.LineSegments(g, materials[i]))
    );
  ...
  redraw() {
    ...
    this.eqDistLines.geometry.addAttribute(
      "position",
      new THREE.Float32BufferAttribute(v2, 3)
    );
    render();
...
function makeTrapsFromSelection() {
  let stiffness = parseFloat(
    (document.getElementById("txtForceValue") as HTMLInputElement).value
  );
...
function makeTrapsFromPairs() {
  ...
  if (nopairs) {
    ask(
      "No basepair info found",
      "Do you want to run an automatic basepair search?",
      () => {
