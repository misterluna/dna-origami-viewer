...
module edit {
  ...
  export function insert(e: BasicElement, sequence: string): BasicElement[] {
    ...
    const getPos = (e) => {
      return e.getInstanceParameter3("bbOffsets");
    };
    ...
    let instanceCopies = added.map((e) => {
      return new InstanceCopy(e);
    });
  ...
  export function ligate(a: BasicElement, b: BasicElement) {
    let end5: BasicElement, end3: BasicElement;
    ...
    if (!a.n5 && !b.n3) {
      ...
      notify(
        "Please select one nucleotide with an available 3' connection and one with an available 5'"
      );
      return;
    }
    ...
    let spRotation = new THREE.Quaternion().setFromUnitVectors(
      ...
      sp.clone().sub(p2).normalize()
    );

    end3.setInstanceParameter("bbconOffsets", sp.toArray());
    end3.setInstanceParameter("bbconRotation", [
      spRotation.w,
      spRotation.z,
      spRotation.y,
      spRotation.x,
   ...
   */
  export function addElementsAt(
    instCopies: InstanceCopy[],
    pos?: THREE.Vector3
  ): BasicElement[] {
    // Add elems
  ...
  export function addElements(instCopies: InstanceCopy[]): BasicElement[] {
    ...
    elems.forEach((e, sid) => {
      ...
      if (c.n3id >= 0) {
        // If we have a 3' neighbor
        let i3 = oldids.findIndex((id) => {
          return id == c.n3id;
        });
      ...
      if (c.n5id >= 0) {
        // If we have a 5' neighbor
        let i5 = oldids.findIndex((id) => {
          return id == c.n5id;
        });
  ...
  export function extendStrand(
    ...
    let addedElems = addElementsBySeq(
      end,
      sequence,
      tmpSys,
      direction,
  ...
  export function extendDuplex(
    ...
    addedElems = addedElems.concat(
      addDuplexBySeq(end, sequence, tmpSys, direction, inverse, 0)
    );
    render();
    return addedElems;
  }
  ...
  export function setSequence(
    elems: Set<BasicElement>,
    sequence: string,
    setComplementaryBases?: boolean
  ) {
    setComplementaryBases = setComplementaryBases || false;
    if (elems.size != sequence.length) {
      notify(
        `You have ${elems.size} particles selected and ${sequence.length} letters in the sequence...doing my best`
      );
    }
    ...
    let set = function () {
      ...
      systems.concat(tmpSystems).forEach((system) => {
        system.nucleoside.geometry["attributes"].instanceColor.needsUpdate =
          true;
  ...
  export function createStrand(
    ...
    tmpSys.initInstances(
      sequence.length * (createDuplex ? 2 : 1) + (createDuplex ? -1 : 0)
    );
    tmpSystems.push(tmpSys);
    ...
    if (createDuplex) {
      ...
      addedElems = addedElems.concat(
        addDuplexBySeq(e, sequence.substring(1), tmpSys, "n3", "n5", 1)
      );
    } else {
      addedElems = addedElems.concat(
        addElementsBySeq(e, sequence.substring(1), tmpSys, "n3", "n5", 1)
      );
    }
    strand.updateEnds();
   ...
   */
  export function discretizeDensity(
    elems: BasicElement[],
    dd: number,
    cellsize: number
  ) {
    ...
    let [xPositions, yPositions, zPositions] = [
      elems.map((e) => e.getPos().x),
      elems.map((e) => e.getPos().y),
      elems.map((e) => e.getPos().z),
    ];
    ...
    let [xGridNum, yGridNum, zGridNum] = [
      Math.ceil((xmax - xmin) / dd),
      Math.ceil((ymax - ymin) / dd),
      Math.ceil((zmax - zmin) / dd),
    ];
    ...
    let get_subbox = function (
      xc,
      yc,
      zc,
      rad
    ): [number[], number[], number[]] {
      //returns slice of density_array (rad**3) and its center
      // minimum of subbox
    ...
    let elem_boxids = Positions.map((V) => {
      return [
        Math.floor((V.x - xmin) / dd),
        Math.floor((V.y - ymin) / dd),
        Math.floor((V.z - zmin) / dd),
      ];
    });
    ...
    for (let i = 0; i < xGridNum; i += 1) {
      for (let j = 0; j < yGridNum; j += 1) {
        for (let k = 0; k < zGridNum; k += 1) {
          density_flat[i + xGridNum * j + xGridNum * yGridNum * k] =
            density_grid[i][j][k];
        }
      }
    }
      ...
      for (let i = min[0]; i < max[0]; i++) {
        for (let j = min[1]; j < max[1]; j++) {
          for (let k = min[2]; k < max[2]; k++) {
            let dist = Math.sqrt((i - cx) ** 2 + (j - cy) ** 2 + (k - cz) ** 2);
            if (dist <= effrad) {
              density_flat[threetoone(min[0] + i, min[1] + j, min[2] + k)] -=
                value;
            } else if (dist > tworad) {
              // density_flat[threetoone(min[0]+x,min[1]+y, min[2]+z)] -= 0.2*1;
            } else {
              // [effrad, tworad]
              density_flat[threetoone(min[0] + 1, min[1] + 2, min[2] + 3)] -=
                0.25 * value;
            }
          }
        }
    ...
    let pos = density_flat.filter(function (a) {
      return a >= 0;
    });
    let postotal = pos.reduce(function (a, b) {
      return a + b;
    });
    ...
    while (postotal > tol) {
      ...
      // find max
      let max = density_flat.reduce(function (a, b) {
        return Math.max(a, b);
      });
      // get index of max
      ...
      pos = density_flat.filter(function (a) {
        return a >= 0;
      });
      postotal = pos.reduce(function (a, b) {
        return a + b;
      });
    }
    ...
    let assignIndices = function (placed) {
      let indices = placed.map(function (a) {
        return [];
      });
      placed.forEach((a, aindx) => {
        ...
        let in_rad = elem_boxids.filter(function (indx) {
          if (min[0] <= indx[0] && indx[0] <= max[0]) {
            // keep elements with correct x dim index
            if (min[1] <= indx[1] && indx[1] <= max[1]) {
              // keep elements with correct y dim index
              if (min[2] <= indx[2] && indx[2] <= max[2]) {
                // keep elements with correct z dim index
  ...
  function copyInstances(
    ...
    destination.fillVec(
      ...
      3,
      id,
      source.getInstanceParameter3("cmOffsets").toArray()
    );
    destination.fillVec(
      "bbOffsets",
      3,
      id,
      source.getInstanceParameter3("bbOffsets").toArray()
    );
    destination.fillVec(
      "nsOffsets",
      3,
      id,
      source.getInstanceParameter3("nsOffsets").toArray()
    );
    destination.fillVec(
      "nsRotation",
      4,
      id,
      source.getInstanceParameter4("nsRotation").toArray()
    );
    destination.fillVec(
      "conOffsets",
      3,
      id,
      source.getInstanceParameter3("conOffsets").toArray()
    );
    destination.fillVec(
      "conRotation",
      4,
      id,
      source.getInstanceParameter4("conRotation").toArray()
    );
    destination.fillVec(
      "bbconOffsets",
      3,
      id,
      source.getInstanceParameter3("bbconOffsets").toArray()
    );
    destination.fillVec(
      "bbconRotation",
      4,
      id,
      source.getInstanceParameter4("bbconRotation").toArray()
    );
    destination.fillVec(
      "bbColors",
      3,
      id,
      source.getInstanceParameter3("bbColors").toArray()
    );
    destination.fillVec(
      "scales",
      3,
      id,
      source.getInstanceParameter3("scales").toArray()
    );
    destination.fillVec(
      "nsScales",
      3,
      id,
      source.getInstanceParameter3("nsScales").toArray()
    );
    destination.fillVec(
      "conScales",
      3,
      id,
      source.getInstanceParameter3("conScales").toArray()
    );
    destination.fillVec(
      "bbconScales",
      3,
      id,
      source.getInstanceParameter3("bbconScales").toArray()
    );
    destination.fillVec(
      "visibility",
      3,
      id,
      source.getInstanceParameter3("visibility").toArray()
    );
    destination.fillVec(
      "nsColors",
      3,
      id,
      source.getInstanceParameter3("nsColors").toArray()
    );
    destination.fillVec(
      "bbLabels",
      3,
      id,
      source.getInstanceParameter3("bbLabels").toArray()
    );
  }
   ...
   */
  export function interconnectDuplex3p(
    strand1: Strand,
    strand2: Strand,
    patch_sequence = "GGGGGGGGG"
  ) {
    let cmss = [];
   ...
   * @param the sequence of the duplex strand
   */
  export function interconnectDuplex5p(
    strand1: Strand,
    strand2: Strand,
    patch_sequence = "GGGGGGGGG"
  ) {
